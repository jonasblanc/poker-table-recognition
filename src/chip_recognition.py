...
def brightness_stats_hsv(img_hsv):
    h,s,v = cv2.split(img_hsv)
    return v.mean(), v.std()

def normalize_brightness_hsv(img_hsv):
    V_MEAN = 180.0
    V_STD = 47.0
    
    img_v_mean, img_v_std = brightness_stats_hsv(img_hsv)
...
def count_chips(chips_rgb, results_dict, plot=False):      
    ...
    chips_hsv = normalize_brightness_hsv(chips_hsv)
    
    color_names = HSV_COLOR_BOUNDS.keys()
    ...
    for i, color_name in enumerate(color_names):
        ...
        for low, high in HSV_COLOR_BOUNDS[color_name]:
        ...
        #for j in range(3):
        #    chips_mask_cleaned = cv2.morphologyEx(chips_mask_cleaned,cv2.MORPH_ERODE,kernel)
        ...
        max_centers = []
        best_threshold = 0
        for treshold in CHIPS_CENTER_INTENSITY_TRESHOLDS:
...
def window_treshold(img, hsv_color_bounds, color, range_idx=0):
    ...
    low_hsv=hsv_color_bounds[color][range_idx][0]
    high_hsv=hsv_color_bounds[color][range_idx][1]
