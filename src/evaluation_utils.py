...
# Utils functions for prediction evaluation and debugging
...
def eval_game_per_category(game_dict, true_game : list , verbose = True):
    ...
    return score_num ,  score_suits, score_chips

def eval_listof_games_per_category( results_games , game_labels ,game_id = [0,1]):
    scores_num = []
    scores_suit = []
    scores_chips = []
    for my_id in game_id:
        ...
        score_num ,  score_suits, score_chips = eval_game_per_category( game_found ,game_true , verbose = True)
        print(f"Num: {score_num}")
        print(f"Suits: {score_suits}")
        print(f"Chips:{score_chips}")

        print("__"*20)
        scores_num.append(score_num)
        scores_suit.append(score_suits)
        scores_chips.append(score_chips)
    
    average_score_num = np.mean(np.array(scores_num))
    average_score_suit = np.mean(np.array(scores_suit))
    average_score_chips = np.mean(np.array(scores_chips))
    print("Average SCORE num = ", average_score_num)    
    print("Average SCORE suits = ", average_score_suit)
    print("Average SCORE chips = ", average_score_chips)
...
def save_results(results= {} , groupid=0):
    ...
    """
    file = f'./predictions/results_group_{str(groupid).zfill(2)}.json'
